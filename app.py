...
st.image(background_image, width = 800)

##input features 
st.subheader("Please input followings details and click on predict button.")
Name = st.text_input("Name")
Current_Loan_Amt =st.number_input("Loan Amount",100)
Term = st.selectbox('loan Term', ['Long', 'short'])
Credit_Score = st.number_input("Credit score", min_value= 0, max_value = 1000 )
Annual_Income = st.number_input("Annual income")
Years_in_current_job = st.slider("Years in current job", 1,10,1)
Home_Ownership = st.selectbox("Home ownership Status", ("Own Home", "Rent", "Home Mortage"))
Monthly_Debt = st.number_input("Outstanding Monthly debt")
Years_of_Credit_History = st.slider("Year of Credit history", 1,70,1)
Num_of_Open_Accts = st.number_input("Number of Account Opened", min_value= 0, max_value = 1000 )
Num_of_Credit_Problems= st.slider("Number of credit card problem", 1,70,1)
Current_Credit_Balance = st.number_input("Current Credit Balance")
Maximum_Open_Credit = st.number_input("Current Credit Limit")
Purpose = st.selectbox("purpose of the Loan", ('Home Improvements', 'Debt Consolidation', 'Buy House', 'other',
...
def predict():
    ...
    if prediction ==1:
        st.success("Dear " + str(Name) + " you are currently eligible for "+  str(Current_Loan_Amt) +  " loan." )
        st.balloons()
        Approval_image  = Image.open('loan approval.jpg')
        st.image(Approval_image, width = 500)
    else:
        st.error("Sorry you are currently not eligible for "+  str(Current_Loan_Amt) +  " loan." )
